...
def replace_max_by_AvgPooling(model):
    ...
    for layer in other_layers:
        if isinstance(layer, tf.keras.layers.MaxPooling2D):
            layer = tf.keras.layers.AveragePooling2D(pool_size=(3,3), strides=(2,2), padding="same")
        x = layer(x)
...
vgg_small.summary()"""
vgg = replace_max_by_AvgPooling(vgg)
vgg.trainable = False
vgg.summary()
...
checkpoint = tf.keras.callbacks.ModelCheckpoint(save_dir+"AvgPooling.keras", 
