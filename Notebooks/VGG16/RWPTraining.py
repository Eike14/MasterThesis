...
def replace_max_by_RWP(model):
    ...
    for layer in other_layers:
        if isinstance(layer, tf.keras.layers.MaxPooling2D):
            layer = RankBasedWeightedPoolingLayer(alpha=0.5, pool_size=3)
        x = layer(x)
...
checkpoint = tf.keras.callbacks.ModelCheckpoint(save_dir+"RWP.keras", 
