...
def replace_max_by_RAP(model):
    ...
    for layer in other_layers:
        if isinstance(layer, tf.keras.layers.MaxPooling2D):
            layer = RankBasedAveragePoolingLayer(threshold=4, pool_size=3)
        x = layer(x)
...
checkpoint = tf.keras.callbacks.ModelCheckpoint(save_dir+"RAP.keras", 
