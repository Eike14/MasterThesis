...
from LpPooling import LpPoolingLayer


print(tf.config.list_physical_devices('GPU'))

data_folder = "C:\\Users\\Eike\\OneDrive\\Desktop\\Uni\\Masterarbeit\\MasterThesisProject\\Dataset\\preprocessed"
# Loading Datasets
...
def replace_max_by_LpPooling(model):
    ...
    for layer in other_layers:
        if isinstance(layer, tf.keras.layers.MaxPooling2D):
            layer = LpPoolingLayer(pool_size=3)
        x = layer(x)
...
vgg_small.summary()"""
vgg = replace_max_by_LpPooling(vgg)
for layer in vgg.layers:
    if not isinstance(layer, LpPoolingLayer):
...
save_dir = "C:\\Users\\Eike\\OneDrive\\Desktop\\Uni\\Masterarbeit\\MasterThesisProject\\Models\\VGG16\\"
...
checkpoint = tf.keras.callbacks.ModelCheckpoint(save_dir+"LpPooling.keras", 
